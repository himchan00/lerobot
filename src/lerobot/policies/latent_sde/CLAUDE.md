...
**Prior `p(z|h)`** (inference + KL target). MLP `LatentPrior` (Gaussian); VQ swaps in `LatentPriorVQ`. The prior always conditions on `h`. `drift_uses_h=False` additionally drops `h` from the **drift** net, so `h` reaches the field *only* through `z` (an `h→z→field` bottleneck; the drift is velocity-blind, reading the current single state frame).
...
- **Dataloader windows** come from the config properties: state gets `[0, horizon)` (current + future demo states — the drift is velocity-blind, so no past frames), images get the past `n_obs` frames (→ `h`), actions get `[0, horizon)` (the chunk targets). `H = horizon` is the training-chunk length; `n_action_steps ≤ horizon` is the deploy execute/refresh period.
